...
function(input, output, session) {
  ...
  observe({
    if(input$FA2_oder_Station=="Clinics"){
    ...
    if(input$FA2_oder_Station=="Units"){
  ...
  observe({
    if(input$FA3_oder_Station=="Clinics"){
      output$FA3_intro<-renderText({"No input file with information on Clinics uploaded."})
    ...
    if(input$FA3_oder_Station=="Units"){
      output$ST3_intro<-renderText({"No input file with information on units uploaded."})
  ...
  observeEvent(input$do_in,{
    ...
    observe({
      if(input$FA2_oder_Station=="Clinics"&&!is.null(input1a)){
        output$FA2_UI1<-renderUI({NULL})
        output$FA2_UI2<-renderUI({NULL})
        output$FA2_UI2a<-renderUI({NULL})
        output$FA2_UI2b<-renderUI({NULL})
        ...
        output$FA_add7a<-renderUI({pickerInput("Min_Kontakt_Tag2",label = HTML("Minimum contact time:&nbsp;&nbsp;&nbsp;"),
                                               choices = c("0 Days","1 Day",paste0(c(2:30)," Days")),selected = "0 Days",
                                               options = list(title="Days",size=5),inline = T,width = "fit")})
        output$FA_add8a<-renderUI({pickerInput("Min_Kontakt_Stunde2",label = "",
                                               choices = c("0 Hours","1 Hour",paste0(c(2:23)," Hours")),selected = "1 Hour",
        ...
        output$FA2_UI1<-renderUI({
          pickerInput('FA2_Fokus',label = "Analysis of Clinics",choices = fa, 
        ...
        observe({
          if(!is.null(input$FA2_Fokus)){
            ...
            output$FA2_UI2a<-renderUI({
              prettyRadioButtons(inputId = "switch0",label = "Filter main diagnosis for ICD code?",
            ...
            observe({
              if(!is.null(input$switch0)){
                if(input$switch0=="no"){
                  ...
                  output$FA2_UI2<-renderUI({dateInput("FA2_Start","Start of observation period",
                  ...
                  if(length(fa2_fokus)==1){
                    output$FA2_UI3b<-renderUI({h5(paste0("Due to the selected clinic, only days between ",
                                                         as.Date(min(input1_fa$Aufnahme)),
                                                         " and ",as.Date(max(input1_fa$Entlassung))," may be selected.\n",
                                                         "Default is a period of 30 days. Hower, this may be changed as required (maximum ",
                                                         as.numeric(as.Date(max(input1_fa$Entlassung))-as.Date(min(input1_fa$Aufnahme))),
                                                         " days). Consider that the results may get complex in case of very long periods."))})
                  }else{
                    output$FA2_UI3b<-renderUI({h5(paste0("Due to the selected clinics, only days between ",
                                                         as.Date(min(input1_fa$Aufnahme)),
                                                         " and ",as.Date(max(input1_fa$Entlassung))," may be selected.\n",
                                                         "Default is a period of 30 days. Hower, this may be changed as required (maximum ",
                                                         as.numeric(as.Date(max(input1_fa$Entlassung))-as.Date(min(input1_fa$Aufnahme))),
                  ...
                  observe({
                    if(!is.null(input$FA2_Start)){
                      if((input$FA2_Start+30)<=as.Date(max(input1_fa$Entlassung))){
                        output$FA2_UI3<-renderUI({dateInput("FA2_Ende","End of observation period",
                  ...
                  if(input$switch0=="ICD chapter"){
                    ...
                    observe({
                      if(!is.null(input$FA2_ICDKapitel)){
                        ...
                        if(nrow(input1_fa)>0){
                          ...
                          if(length(fa2_fokus)==1){
                            output$FA2_UI3b<-renderUI({h5(paste0("Due to the selected clinic and ICD filter, only days between ",
                                                                 as.Date(min(input1_fa$Aufnahme)),
                                                                 " and ",as.Date(max(input1_fa$Entlassung))," may be selected.\n",
                                                                 "Default is a period of 30 days. Hower, this may be changed as required (maximum ",
                                                                 as.numeric(as.Date(max(input1_fa$Entlassung))-as.Date(min(input1_fa$Aufnahme))),
                            ...
                            output$FA2_UI3b<-renderUI({h5(paste0("Due to the selected clinics and ICD filter, only days between ",
                                                                 as.Date(min(input1_fa$Aufnahme)),
                                                                 " and ",as.Date(max(input1_fa$Entlassung))," may be selected.\n",
                                                                 "Default is a period of 30 days. Hower, this may be changed as required (maximum ",
                                                                 as.numeric(as.Date(max(input1_fa$Entlassung))-as.Date(min(input1_fa$Aufnahme))),
                          ...
                          if(length(fa2_fokus)==1){
                            output$FA2_UI3b<-renderUI({h5(paste0("For the selected clinic, no cases are available with the selected ICD filter.
                                                                 Pick another clinic and/or change your ICD filter, to be able to select an  
                                                                 observation period."))})
                            output$FA2_UI2<-renderUI({NULL})
                            output$FA2_UI3<-renderUI({NULL})
                          }else{
                            output$FA2_UI3b<-renderUI({h5(paste0("For the selected clinics, no cases are available with the selected ICD filter.
                                                                 Pick another clinic and/or change your ICD filter, to be able to select an  
                                                                 observation period."))})
                            output$FA2_UI2<-renderUI({NULL})
                            output$FA2_UI3<-renderUI({NULL})
                    ...
                    output$FA2_UI2c<-renderUI({
                      textInput("FA2_ICDExakt",value = NULL,placeholder = "e.g. A00 or A00.0",label = NULL)
                    })
                    output$FA2_UI2b<-renderUI({NULL})
    ...
    observe({
      if(input$FA2_oder_Station=="Units"&&!is.null(input1b)){
        output$FA2_UI1<-renderUI({NULL})
        output$FA2_UI2<-renderUI({NULL})
        output$FA2_UI2a<-renderUI({NULL})
        output$FA2_UI2b<-renderUI({NULL})
        ...
        output$ST2_UI1<-renderUI({
          pickerInput('ST2_Fokus',label = "Analysis of Units",choices = fa, 
                      options = list(`actions-box` = TRUE,
                                     `deselect-all-text` = "Reset selection",
                                     `select-all-text` = "Select all",
                                     `none-selected-text` = "Nothing selected"),multiple=TRUE)
        ...
        observe({
          if(!is.null(input$ST2_Fokus)){
            ...
            output$ST2_UI2a<-renderUI({
              prettyRadioButtons(inputId = "switch0c",label = "Filter main diagnosis for ICD code?",
                                 choices = c("no","ICD chapter","exact ICD code"),selected = "no",
                                 inline=T,outline = T,bigger = T,
                                 status = "primary",
                                 icon=icon("check")
            ...
            observe({
              if(!is.null(input$switch0c)){
                if(input$switch0c=="no"){
                  output$ST2_UI2b<-renderUI({NULL})
                  output$ST2_UI2c<-renderUI({NULL})
                  
                  output$ST2_UI2<-renderUI({dateInput("ST2_Start","Start of observation period",
                                                      value=as.Date(min(input1_fa$Aufnahme),format="yyyy-mm-dd"),
                                                      min = as.Date(min(input1_fa$Aufnahme),format="yyyy-mm-dd"),
                                                      max = as.Date(max(input1_fa$Entlassung),format="yyyy-mm-dd"),
                                                      weekstart = 1,language = "de",format="yyyy-mm-dd")})
                  ...
                  if(length(fa2_fokus)==1){
                    output$ST2_UI3b<-renderUI({h5(paste0("Due to the selected unit, only days between ",
                                                         as.Date(min(input1_fa$Aufnahme)),
                                                         " and ",as.Date(max(input1_fa$Entlassung))," may be selected.\n",
                                                         "Default is a period of 30 days. However, this may be changed as required (maximum ",
                                                         as.numeric(as.Date(max(input1_fa$Entlassung))-as.Date(min(input1_fa$Aufnahme))),
                                                         " days). Consider that the results may get complex in case of very long periods."))})
                  }else{
                    output$ST2_UI3b<-renderUI({h5(paste0("Due to the selected units, only days between ",
                                                         as.Date(min(input1_fa$Aufnahme)),
                                                         " and ",as.Date(max(input1_fa$Entlassung))," may be selected.\n",
                                                         "Default is a period of 30 days. However, this may be changed as required (maximum ",
                                                         as.numeric(as.Date(max(input1_fa$Entlassung))-as.Date(min(input1_fa$Aufnahme))),
                  ...
                  observe({
                    if(!is.null(input$ST2_Start)){
                      if((input$ST2_Start+30)<=as.Date(max(input1_fa$Entlassung))){
                        output$ST2_UI3<-renderUI({dateInput("ST2_Ende","End of observation period",
                  ...
                  if(input$switch0c=="ICD chapter"){
                    ...
                    observe({
                      if(!is.null(input$ST2_ICDKapitel)){
                        ...
                        if(nrow(input1_fa)>0){
                          ...
                          if(length(fa2_fokus)==1){
                            output$ST2_UI3b<-renderUI({h5(paste0("For the selected unit, no cases are available with the selected ICD filter.
                                                                 Pick another unit and/or change your ICD filter, to be able to select an  
                                                                 observation period."))})
                            output$ST2_UI2<-renderUI({NULL})
                            output$ST2_UI3<-renderUI({NULL})
                          }else{
                            output$ST2_UI3b<-renderUI({h5(paste0("For the selected units, no cases are available with the selected ICD filter.
                                                                 Pick another units and/or change your ICD filter, to be able to select an  
                                                                 observation period."))})
                            output$ST2_UI2<-renderUI({NULL})
                            output$ST2_UI3<-renderUI({NULL})
                          }
                    ...
                    output$ST2_UI2c<-renderUI({
                      textInput("ST2_ICDExakt",value = NULL,placeholder = "z.B. A00 or A00.0",label = NULL)
                    })
                    output$ST2_UI2b<-renderUI({NULL})
      ...
      if(input$FA2_oder_Station=="Units"&&is.null(input1b)){
        ...
        output$ST2_UI4<-renderUI({NULL})
        output$ST2_UI4b<-renderUI({NULL})
        output$ST_intro<-renderText({"No input file with information on clinics uploaded."})
    ...
    output$FAST2_UI5<-renderUI({actionBttn("do_superspreader",label = "Start Analysis",style = "gradient",color = "primary",disabled=T)})
    
    observe({
      if((!is.null(input1a)&&!is.null(input$FA2_Fokus)&&!is.null(input$switch0)&&
          (input$switch0=="no"|(input$switch0=="ICD chapter"&&!is.null(input$FA2_ICDKapitel))|(!is.null(input$FA2_ICDExakt)&&input$FA2_ICDExakt!=""&&nchar(input$FA2_ICDExakt)>=3)))||
         (!is.null(input1b)&&!is.null(input$ST2_Fokus)&&!is.null(input$switch0c)&&
          (input$switch0c=="no"|(input$switch0c=="ICD chapter"&&!is.null(input$ST2_ICDKapitel))|(!is.null(input$ST2_ICDExakt)&&input$ST2_ICDExakt!=""&&nchar(input$ST2_ICDExakt)>=3)))){
        if(input$FA2_oder_Station=="Clinics"&&!is.null(input1a)){
    ...
    observeEvent(input$do_superspreader,{
      ...
      if(input$FA2_oder_Station=="Clinics"){
        ...
        if(input$switch0=="exact ICD code"){
          user_icd<-gsub(".","\\.",input$FA2_ICDExakt,fixed=T)
          input1_fa<-input1_fa[grep(paste0("^",user_icd),input1_fa$ICDfull),]
      ...
      if(input$FA2_oder_Station=="Units"){
        ...
        if(input$switch0c=="exact ICD code"){
          user_icd<-gsub(".","\\.",input$ST2_ICDExakt,fixed=T)
          input1_fa<-input1_fa[grep(paste0("^",user_icd),input1_fa$ICDfull),]
      ...
      ##Filter Zeitraum
      if(kontaktzeit==0){
        output$text_analyse1<-renderUI({"Please select the minimum contact time."})
        output$text_analyse2<-renderUI({NULL})
        output$force<-renderForceNetwork({NULL})
        return()
      }
      if((FA_Ende-FA_Start)<kontaktzeit){
        output$text_analyse1<-renderUI(HTML("The selected observation period is shorter than the minimum contact time.<br>
        Please select a shorter observation period or a shorter minimum contact time."))
        output$text_analyse2<-renderUI({NULL})
        output$force<-renderForceNetwork({NULL})
        return()
      }
      ...
      if(length(fall_in)>0){
        ...
        progress <- shiny::Progress$new()
        progress$set(message = "Process patient information", value = 0)
        ...
        progress <- shiny::Progress$new()
        progress$set(message = ifelse(input$FA2_oder_Station=="Clinics",
                                      "Process contacts between clinics",
                                      "Process contacts between units"), value = 0)
        ...
        #script <-'alert("Case: " + (d.NodeID) + "\\n \\n Contacts altogether: " + (d.Count_ges) + "\\n Length of stay altogether: " + (d.Dauer_ges) + " Days\\n \\n Contacts at clinic " + (d.Group) + ": " + (d.Count_einzeln) + "\\n Length of stay at clinic " + (d.Group) + ": " + (d.Dauer) + " Days \\n \\n Weitere besuchte Clinic: " + (d.Weitere));'
        script <-'alert(d.Test)'
        #}else{
        #script <-'alert("Case: " + (d.NodeID) + "\\n \\n Contacts altogether: " + (d.Count_ges) + "\\n Length of stay altogether: " + (d.Dauer_ges) + " Days\\n \\n Contacts at unit " + (d.Group) + ": " + (d.Count_einzeln) + "\\n Length of stay at unit " + (d.Group) + ": " + (d.Dauer) + " Days \\n \\n Further units visited: " + (d.Weitere));'
        #}
        #message(nrow(links1))
        ...
        if((innerhalb==F)){#|(nrow(links1_backup[links1_backup$Farbe=="gray85",])-nrow(nodes1))>10000)&length(fa)!=1){
          ...
          progress <- shiny::Progress$new()
          progress$set(message = ifelse(input$FA2_oder_Station=="Clinics",
                                        "Analyze other clinics",
                                        "Analyze other units"), value = 0)
          abt<-fa
          andere<-nodes1[!is.na(nodes1$Weitere)&nodes1$Weitere!="-"&nodes1$Group==abt,]
          if(nrow(andere)>0){
            andere_alle<-input1_ohnefilter2[input1_ohnefilter2$Fallnummer%in%andere$NodeID,]
        ...
        if(input$FA2_oder_Station=="Clinics"){
          if(length(fa)==1){
            fn$x$nodes$Test<-paste0("Case: ",fn$x$nodes$NodeID,"\nMain diagnosis: ",
                                    fn$x$nodes$ICDfull,
                                    "\n\nLength of stay altogether: ",
                                    fn$x$nodes$Dauer_ges," Days\n \nContacts at clinic ",
                                    fn$x$nodes$Group,": ",fn$x$nodes$Count_einzeln,
                                    "\nLength of stay at clinic ",fn$x$nodes$Group,": ",
                                    fn$x$nodes$Dauer," Days \n \nFurther clinics visited: ",fn$x$nodes$Weitere) 
          }else{
            fn$x$nodes$Test<-paste0("Case: ",fn$x$nodes$NodeID,"\nMain diagnosis: ",
                                    fn$x$nodes$ICDfull,"\n \nContacts altogether: ",
          ...
          fn$x$nodes$Test[fn$x$nodes$name==fn$x$nodes$group]<-paste0("Clinic: ",
                                                                     fn$x$nodes$NodeID[fn$x$nodes$name==fn$x$nodes$group],
                                                                     "\n\nCases altogether: ",
                                                                     fn$x$nodes$NodeSize[fn$x$nodes$name==fn$x$nodes$group],
                                                                     "\nCases with transfer from ",fn$x$nodes$NodeID[fn$x$nodes$name==fn$x$nodes$group],": ",
                                                                     fn$x$nodes$Count_einzeln[fn$x$nodes$name==fn$x$nodes$group],
                                                                     "\nTop-5 to: ",fn$x$nodes$NodeSize2[fn$x$nodes$name==fn$x$nodes$group],
                                                                     "\n\nCases with transfer to ",fn$x$nodes$NodeID[fn$x$nodes$name==fn$x$nodes$group],": ",
                                                                     fn$x$nodes$Count_ges[fn$x$nodes$name==fn$x$nodes$group],
                                                                     "\nTop-5 from: ",fn$x$nodes$NodeSize2b[fn$x$nodes$name==fn$x$nodes$group],
                                                                     "\n\nAverage length of stay altogether: ",fn$x$nodes$Dauer_ges[fn$x$nodes$name==fn$x$nodes$group]," Days",
                                                                     "\nAverage length of stay at ",fn$x$nodes$NodeID[fn$x$nodes$name==fn$x$nodes$group],": ",
                                                                     fn$x$nodes$Dauer[fn$x$nodes$name==fn$x$nodes$group]," Days"
          )
        }else{
          if(length(fa)==1){
            fn$x$nodes$Test<-paste0("Case: ",fn$x$nodes$NodeID,"\nMain diagnosis: ",
                                    fn$x$nodes$ICDfull,"\n\n \nContacts at unit ",
                                    fn$x$nodes$Group,": ",fn$x$nodes$Count_einzeln,
                                    "\nLength of stay at unit ",fn$x$nodes$Group,": ",
                                    fn$x$nodes$Dauer," Days \n \nFurther units visited: ",fn$x$nodes$Weitere) 
          }else{
            fn$x$nodes$Test<-paste0("Case: ",fn$x$nodes$NodeID,"\nMain diagnosis: ",
                                    fn$x$nodes$ICDfull,"\n \nContacts altogether: ",
                                    fn$x$nodes$Count_ges,"\nLength of stay altogether: ",
          ...
          fn$x$nodes$Test[fn$x$nodes$name==fn$x$nodes$group]<-paste0("Unit: ",
                                                                     fn$x$nodes$NodeID[fn$x$nodes$name==fn$x$nodes$group],
                                                                     "\n\nCases altogether: ",
                                                                     fn$x$nodes$NodeSize[fn$x$nodes$name==fn$x$nodes$group],
                                                                     "\nCases with transfer from ",fn$x$nodes$NodeID[fn$x$nodes$name==fn$x$nodes$group],": ",
...
'
    ...
    simulation.restart();
        ...
        if(input$FA2_oder_Station=="Clinics"){
          if(min_kontakt_tag!=1&min_kontakt_stunde!=1){
            if(length(fa2_fokus)!=1){
              output$text_analyse2<-renderUI(HTML(paste0("Clinics selected: ",paste0(fa2_fokus,collapse = ", "),
                                                         "<br>Clinics included in analysis: ",paste0(fa,collapse = ", "),
                                                         "<br>ICD filter: ",paste0(icd_help_out[as.numeric(FA2_ICD)],collapse=", "),
                                                         "<br><br>Observation period: ",as.Date(FA_Start)," to ",as.Date(FA_Ende),
                                                         "<br>Minimum contact time: ",min_kontakt_tag," Days and ",min_kontakt_stunde," Hours",
                                                         #"<br>Minimale Kontakt-Anzahl: ",min_kontakt,
                                                         "<br><br>Cases: ",nrow(nodes1_backup),
                                                         "<br>Contacts within the Clinics ",
                                                         ...
                                                         "<br>Transfers between clinics: ",nrow(links1_backup[links1_backup$Farbe!="gray85",]),
                                                         "<br><br>Analysis successfully conducted")))
            }else{
              output$text_analyse2<-renderUI(HTML(paste0("Clinic selected: ",paste0(fa2_fokus,collapse = ", "),
                                                         "<br>ICD filter: ",paste0(icd_help_out[as.numeric(FA2_ICD)],collapse=", "),
                                                         "<br><br>Observation period: ",as.Date(FA_Start)," to ",as.Date(FA_Ende),
                                                         "<br>Minimum contact time: ",min_kontakt_tag," Days and ",min_kontakt_stunde," Hours",
                                                         #"<br>Minimale Kontakt-Anzahl: ",min_kontakt,
                                                         "<br><br>Cases: ",nrow(nodes1_backup),
                                                         "<br><br>Contacts within the Clinic: ",nrow(links1_backup[links1_backup$Farbe=="gray85",]),
                                                         "<br><br>Cases with transfer from ",fa2_fokus,": ",von_summe,
                                                         "<br>Top-5 to: ",von5,
                                                         "<br><br>Cases with transfer to ",fa2_fokus,": ",nach_summe,
                                                         "<br>Top-5 from: ",nach5,
                                                         "<br><br>Analysis successfully conducted")))
            }
          }
          if(min_kontakt_tag==1&min_kontakt_stunde!=1){
            if(length(fa2_fokus)!=1){
              output$text_analyse2<-renderUI(HTML(paste0("Clinics selected: ",paste0(fa2_fokus,collapse = ", "),
                                                         ...
                                                         "<br>ICD filter: ",paste0(icd_help_out[as.numeric(FA2_ICD)],collapse=", "),
                                                         "<br><br>Observation period: ",as.Date(FA_Start)," to ",as.Date(FA_Ende),
                                                         "<br>Minimum contact time: ",min_kontakt_tag," Day and ",min_kontakt_stunde," Hours",
                                                         #"<br>Minimale Kontakt-Anzahl: ",min_kontakt,
                                                         "<br><br>Cases: ",nrow(nodes1_backup),
                                                         "<br>Contacts within the Clinics ",
                                                         #ifelse(((innerhalb==F|(nrow(links1_backup[links1_backup$Farbe=="gray85",])-nrow(nodes1)-length(fa))>10000)&length(fa)!=1),"(nicht dargestellt): ","(dargestellt): "),
          ...
          if(min_kontakt_tag!=1&min_kontakt_stunde==1){
            if(length(fa2_fokus)!=1){
              output$text_analyse2<-renderUI(HTML(paste0("Clinics selected: ",paste0(fa2_fokus,collapse = ", "),
                                                         ...
                                                         "<br>ICD filter: ",paste0(icd_help_out[as.numeric(FA2_ICD)],collapse=", "),
                                                         "<br><br>Observation period: ",as.Date(FA_Start)," to ",as.Date(FA_Ende),
                                                         "<br>Minimum contact time: ",min_kontakt_tag," Days and ",min_kontakt_stunde," Hour",
                                                         #"<br>Minimale Kontakt-Anzahl: ",min_kontakt,
                                                         "<br><br>Cases: ",nrow(nodes1_backup),
                                                         "<br>Contacts within the Clinics ",
                                                         #ifelse(((innerhalb==F|(nrow(links1_backup[links1_backup$Farbe=="gray85",])-nrow(nodes1)-length(fa))>10000)&length(fa)!=1),"(nicht dargestellt): ","(dargestellt): "),
          ...
          if(min_kontakt_tag==1&min_kontakt_stunde==1){
            if(length(fa2_fokus)!=1){
              output$text_analyse2<-renderUI(HTML(paste0("Clinics selected: ",paste0(fa2_fokus,collapse = ", "),
                                                         ...
                                                         "<br>ICD filter: ",paste0(icd_help_out[as.numeric(FA2_ICD)],collapse=", "),
                                                         "<br><br>Observation period: ",as.Date(FA_Start)," to ",as.Date(FA_Ende),
                                                         "<br>Minimum contact time: ",min_kontakt_tag," Day and ",min_kontakt_stunde," Hour",
                                                         #"<br>Minimale Kontakt-Anzahl: ",min_kontakt,
                                                         "<br><br>Cases: ",nrow(nodes1_backup),
                                                         "<br>Contacts within the Clinics ",
                                                         #ifelse(((innerhalb==F|(nrow(links1_backup[links1_backup$Farbe=="gray85",])-nrow(nodes1)-length(fa))>10000)&length(fa)!=1),"(nicht dargestellt): ","(dargestellt): "),
              ...
              output$text_analyse2<-renderUI(HTML(paste0("Fachabteilung selected: ",paste0(fa2_fokus,collapse = ", "),
                                                         "<br>ICD filter: ",paste0(icd_help_out[as.numeric(FA2_ICD)],collapse=", "),
                                                         "<br><br>Observation period: ",as.Date(FA_Start)," to ",as.Date(FA_Ende),
                                                         "<br>Minimum contact time: ",min_kontakt_tag," Day and ",min_kontakt_stunde," Hour",
                                                         #"<br>Minimale Kontakt-Anzahl: ",min_kontakt,
          ...
          if(min_kontakt_tag!=1&min_kontakt_stunde!=1){
            if(length(fa2_fokus)!=1){
              output$text_analyse2<-renderUI(HTML(paste0("Units selected: ",paste0(fa2_fokus,collapse = ", "),
                                                         "<br>Units included in analysis: ",paste0(fa,collapse = ", "),
                                                         "<br>ICD filter: ",paste0(icd_help_out[as.numeric(FA2_ICD)],collapse=", "),
                                                         "<br><br>Observation period: ",as.Date(FA_Start)," to ",as.Date(FA_Ende),
                                                         "<br>Minimum contact time: ",min_kontakt_tag," Days and ",min_kontakt_stunde," Hours",
                                                         #"<br>Minimale Kontakt-Anzahl: ",min_kontakt,
                                                         "<br><br>Cases: ",nrow(nodes1_backup),
                                                         "<br>Contacts within the Units ",
                                                         #ifelse(((innerhalb==F|(nrow(links1_backup[links1_backup$Farbe=="gray85",])-nrow(nodes1)-length(fa))>10000)&length(fa)!=1),"(nicht dargestellt): ","(dargestellt): "),
                                                         #(nrow(links1_backup[links1_backup$Farbe=="gray85",])-nrow(nodes1)-length(fa)),
                                                         sum(as.numeric(nodes1_backup$Count_einzeln))/2,
                                                         "<br>Transfers between units: ",nrow(links1_backup[links1_backup$Farbe!="gray85",]),
                                                         "<br><br>Analysis successfully conducted")))
            }else{
              output$text_analyse2<-renderUI(HTML(paste0("Unit selected: ",paste0(fa2_fokus,collapse = ", "),
                                                         "<br>ICD filter: ",paste0(icd_help_out[as.numeric(FA2_ICD)],collapse=", "),
                                                         "<br><br>Observation period: ",as.Date(FA_Start)," to ",as.Date(FA_Ende),
                                                         "<br>Minimum contact time: ",min_kontakt_tag," Days and ",min_kontakt_stunde," Hours",
                                                         #"<br>Minimale Kontakt-Anzahl: ",min_kontakt,
        ...
        output$text_analyse1<-renderUI(HTML("With the selected configuration (minimum contact time ",round(kontaktzeit,2)," days) there are no cases left.<br>
        Please select a shorter minimum contact time."))
        output$text_analyse2<-renderUI({NULL})
        output$force<-renderForceNetwork({NULL})
        #output$plotBasic<-renderPlot({NULL})
        return()
    ...
    observe({
      if(input$FA3_oder_Station=="Clinics"&&!is.null(input1a)){
        ...
        output$FA3_intro<-renderText({NULL})
        output$ST3_intro<-renderText({NULL})
        output$FA3_add1a<-renderUI({prettyRadioButtons(inputId = "switch1",label = "Transfers",
                                                      choices = c("from","to"),selected = "from",
                                                      inline=T,outline = T,bigger = T,
                                                      status = "primary",
                                                      icon=icon("check"))})
        output$FA3_add2a<-renderText({"Select observation period"})
        ...
        output$FA3_UI2<-renderUI({
          pickerInput('FA3_Fokus',label = "Clinic",choices = fa,selected = fa[1],options = list(`live-search` = TRUE))
        })
        output$ST3_UI2<-renderUI({NULL})
        ...
        observe({
          if(!is.null(input$FA3_Fokus)){
            ...
            output$FA3_UI2a<-renderUI({
              prettyRadioButtons(inputId = "switch1b",label = "Filter main diagnosis for ICD code?",
                                 choices = c("no","ICD chapter","exact ICD code"),selected = "no",
                                 inline=T,outline = T,bigger = T,
                                 status = "primary",
                                 icon=icon("check")
            ...
            observe({
              if(!is.null(input$switch1b)){
                ...
                if(input$switch1b=="no"){
                  output$FA3_UI2b<-renderUI({NULL})
                  output$FA3_UI2c<-renderUI({NULL})
                  
                  output$FA3_UI4<-renderUI({dateInput("FA3_Start","Start of observation period",
                                                      value=as.Date(min(input1_fa$Aufnahme),format="yyyy-mm-dd"),
                                                      min = as.Date(min(input1_fa$Aufnahme),format="yyyy-mm-dd"),
                                                      max = as.Date(max(input1_fa$Entlassung),format="yyyy-mm-dd"),
                                                      weekstart = 1,language = "de",format="yyyy-mm-dd")})
                  ...
                  observe({
                    if(!is.null(input$FA3_Start)){
                      output$FA3_UI5<-renderUI({dateInput("FA3_Ende","End of observation period",
                  ...
                  if(input$switch1b=="ICD chapter"){
                    ...
                    output$FA3_UI2c<-renderUI({
                      textInput("FA3_ICDExakt",value = NULL,placeholder = "e.g. A00 or A00.0",label = NULL)
                    })
                    output$FA3_UI2b<-renderUI({NULL})
                    output$ST3_UI2b<-renderUI({NULL})
                    output$ST3_UI2c<-renderUI({NULL})
    ...
    #####################pro Units
    observe({
      if(input$FA3_oder_Station=="Units"&&!is.null(input1b)){
        output$FA_erweitert_UI2<-renderUI({NULL})
        ...
        output$ST3_UI2<-renderUI({
          pickerInput('ST3_Fokus',label = "Unit",choices = fa,selected=fa[1],options = list(`live-search` = TRUE))
        })
        output$FA3_UI2<-renderUI({NULL})
        ...
        observe({
          if(!is.null(input$ST3_Fokus)){
            ...
            output$ST3_UI2a<-renderUI({
              prettyRadioButtons(inputId = "switch1c",label = "Filter main diagnosis for ICD code?",
                                 choices = c("no","ICD chapter","exact ICD code"),selected = "no",
                                 inline=T,outline = T,bigger = T,
                                 status = "primary",
                                 icon=icon("check")
            ...
            observe({
              if(!is.null(input$switch1c)){
                if(input$switch1c=="no"){
                  output$ST3_UI2b<-renderUI({NULL})
                  output$ST3_UI2c<-renderUI({NULL})
                  output$FA3_UI2b<-renderUI({NULL})
                  output$FA3_UI2c<-renderUI({NULL})
                  
                  output$ST3_UI4<-renderUI({dateInput("ST3_Start","Start of observation period",
                                                      value=as.Date(min(input1_fa$Aufnahme),format="yyyy-mm-dd"),
                                                      min = as.Date(min(input1_fa$Aufnahme),format="yyyy-mm-dd"),
                                                      max = as.Date(max(input1_fa$Entlassung),format="yyyy-mm-dd"),
                                                      weekstart = 1,language = "de",format="yyyy-mm-dd")})
                  ...
                  observe({
                    if(!is.null(input$ST3_Start)){
                      output$ST3_UI5<-renderUI({dateInput("ST3_Ende","End of observation period",
                  ...
                  if(input$switch1c=="ICD chapter"){
                    ...
                    output$ST3_UI2c<-renderUI({
                      textInput("ST3_ICDExakt",value = NULL,placeholder = "e.g. A00 or A00.0",label = NULL)
                    })
                    output$ST3_UI2b<-renderUI({NULL})
                    output$FA3_UI2b<-renderUI({NULL})
                    output$FA3_UI2c<-renderUI({NULL})
      ...
      if(input$FA3_oder_Station=="Units"&&is.null(input1b)){
        ...
        output$ST3_intro<-renderText({"No input file with information on clinics uploaded."})
        output$FA3_intro<-renderText({NULL})
        output$FA3_add1a<-renderUI({NULL})
        output$FA3_add2a<-renderText({NULL})
      }
    ...
    output$FAST3_UI5<-renderUI({actionBttn("do_zusammenfassung",label = "Start Analysis",style = "gradient",color = "primary",disabled=T)})
    
    observe({
      if((!is.null(input$FA3_Fokus)&&!is.null(input$switch1b)&&(input$switch1b=="no"|(input$switch1b=="ICD chapter"&&!is.null(input$FA3_ICDKapitel))|(!is.null(input$FA3_ICDExakt)&&input$FA3_ICDExakt!=""&&nchar(input$FA3_ICDExakt)>=3)))||
         (!is.null(input$ST3_Fokus)&&!is.null(input$switch1c)&&(input$switch1c=="no"|(input$switch1c=="ICD chapter"&&!is.null(input$ST3_ICDKapitel))|(!is.null(input$ST3_ICDExakt)&&input$ST3_ICDExakt!=""&&nchar(input$ST3_ICDExakt)>=3)))){
        if(input$FA2_oder_Station=="Clinics"&&!is.null(input1a)){
    ...
    observeEvent(input$do_zusammenfassung,{
      ...
      if(input$FA3_oder_Station=="Clinics"){
        ...
        if(input$switch1b=="exact ICD code"){
          user_icd<-gsub(".","\\.",input$FA3_ICDExakt,fixed=T)
      ...
      if(input$FA3_oder_Station=="Units"){
        ...
        if(input$switch1c=="exact ICD code"){
          user_icd<-gsub(".","\\.",input$ST3_ICDExakt,fixed=T)
          input1<-input1[grep(paste0("^",user_icd),input1$ICDfull),] 
      ...
      if(nrow(input1_fa_filtered)>1&&length(unique(input1_fa_filtered$Abteilung))>1){
        ...
        progress <- shiny::Progress$new()
        progress$set(message = ifelse(input$FA3_oder_Station=="Clinics","Analyze clinic","Analyze unit"), value = 0)
        ...
        for(i in 1:nrow(links1)){
          ...
          ifelse(input$switch1=="from",links1$Valueb[i]<-links1$Value[i]/links1$Source_Anteil[i],links1$Valueb[i]<-links1$Value[i]/links1$Target_Anteil[i])
          ifelse(input$switch1=="from",links1$Valueb2[i]<-links1$Value[i]/links1$Source_Total[i],links1$Valueb2[i]<-links1$Value[i]/links1$Target_Total[i])
        }
        ...
        if(input$switch1=="from"){
          script <-'alert((d.Referenz) + "\\n\\n Cases altogether: " + (d.Referenz_Count) + "\\n Cases transferred: " + (d.Referenz_Anteil) + "\\n \\n" + "From " + (d.Referenz) + " to " + (d.NodeID) + ": " + (d.Nodes_Value) + "/" + (d.Referenz_Anteil) + "\\n\\nEffective distance:\\n     " + (d.Distanz) + " (based on transferred cases)" + "\\n     " + (d.Distanz2) + " (based on all cases)" );'
        }else{
          script <-'alert((d.Referenz) + "\\n\\n Cases altogether: " + (d.Referenz_Count) + "\\n Cases transferred: " + (d.Referenz_Anteil) + "\\n \\n" + "From " + (d.NodeID) + " to " + (d.Referenz) + ": " + (d.Nodes_Value) + "/" + (d.Referenz_Anteil) + "\\n\\nEffective distance:\\n     " + (d.Distanz) + " (based on transferred cases)" + "\\n     " + (d.Distanz2) + " (based on all cases)" );'
        }
        ...
        if(input$FA3_oder_Station=="Clinics"){
          if(input$switch1=="from"){
            output$text_analyse2<-renderUI(HTML(paste0("Transfers",
                                                       "<br><br>From: ",FA_Fokus,
                                                       "<br>To: ",paste0(links1$Target,collapse = ", "),
                                                       "<br>ICD filter: ",paste0(icd_help_out[as.numeric(FA3_ICD)],collapse=", "),
                                                       "<br><br>Observation period: ",as.Date(FA_Start)," to ",as.Date(FA_Ende),
                                                       "<br><br>Analysis successfully conducted")))
          }else{
            output$text_analyse2<-renderUI(HTML(paste0("Transfers",
                                                       "<br><br>From: ",paste0(links1$Source,collapse = ", "),
                                                       "<br>To: ",FA_Fokus,
                                                       "<br>ICD filter: ",paste0(icd_help_out[as.numeric(FA3_ICD)],collapse=", "),
                                                       "<br><br>Observation period: ",as.Date(FA_Start)," to ",as.Date(FA_Ende),
                                                       "<br><br>Analysis successfully conducted")))
        ...
        output$text_analyse1<-renderUI(HTML("For the selected observation period, no cases are available.<br>
        Please select a longer observation period and/or change your ICD filter."))
        output$text_analyse2<-renderUI({NULL})
        output$force<-renderForceNetwork({NULL})
        return()
      }
      ...
      rv2$nodes1<-nodes1
      rv2$links1<-links1
      rv2$fa<-ifelse(input$FA3_oder_Station=="Clinics",FA_Fokus,ST_Fokus)
      ...
      output$do_zusammenfassung_export <- downloadHandler(
        filename = function() {
          paste0("Summary_transfers_",ifelse(input$switch1=="from","from_","to_"),
        ...
        content = function(file){
          ...
          add_data<-data.frame(V1=c("","Summary TRACE: Transferchains and contacts in hospitals",
                                    ifelse(input$switch1=="from","From: ","To: "),
                                    "ICD filter: ",
                                    "Observation period: ",
                                    "Cases altogether: ",
                                    "Transferred cases: ",
                                    "",
                                    ifelse(input$switch1=="from",
                                           paste0("Transferred from ",fa," to "),
                                           paste0("Transferred to ",fa," from ")),
                                    ""),
                               V2=c("","",
                                    fa,
                                    paste0(icd_help_out[as.numeric(FA3_ICD)],collapse=", "),
                                    paste0(as.Date(FA_Start)," to ",as.Date(FA_Ende)),
